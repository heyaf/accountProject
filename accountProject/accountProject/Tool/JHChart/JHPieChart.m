...
//
            ...
            if (_didClickType == JHPieChartDidClickTranslateToBig) {
                ...
                [self.showInfoView updateFrameTo:CGRectMake(p.x, p.y, self.showInfoView.frame.size.width, self.showInfoView.frame.size.height) andBGColor:colors[i%colors.count] andShowContentString:[NSString stringWithFormat:@"%@ 共:% 3ld 占:%.1f%c",weakself.descArr[i],[self.valueArr[i] integerValue],present,'%']];
                return;
            }
