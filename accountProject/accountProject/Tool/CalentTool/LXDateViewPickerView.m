...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    return [self initWithFrame:frame
                     stareDate:[NSDate date]
                       endDate:[[NSDate date] dateAfterDay:0]];
}
...
- (void)showDatePickerViewControllerWithSelectedDate:(NSDate *)date uploadLabel:(UILabel *)label {
    ...
    if (self.delegate && [self.delegate isKindOfClass:[UIViewController class]]) {
        ...
        pickerVC.selectBlock = ^(NSString *dateString) {
            ...
            if ([weakSelf.delegate respondsToSelector:@selector(dateViewPickerView:didSelectedDate:AndisStartLB:)]) {
                if (label.tag ==12138) {
                    [weakSelf.delegate dateViewPickerView:weakSelf didSelectedDate:[NSDate dateFromString:dateString withFormat:kDateFormate_1] AndisStartLB:YES];
                }else{
                    
                    [weakSelf.delegate dateViewPickerView:weakSelf didSelectedDate:[NSDate dateFromString:dateString withFormat:kDateFormate_1] AndisStartLB:NO];
                }
...
- (UIButton *)stareCalendarIcon {
    ...
    if (!_stareCalendarIcon) {
        ...
        [_stareCalendarIcon setBackgroundImage:[UIImage imageNamed:@"icon_calendar"] forState:UIControlStateNormal];
    }
...
- (UIButton *)endCalendarIcon {
    ...
    if (!_endCalendarIcon) {
        ...
        [_endCalendarIcon setBackgroundImage:[UIImage imageNamed:@"icon_calendar"] forState:UIControlStateNormal];
    }
